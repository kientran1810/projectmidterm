...
public class ExManagerUI extends JFrame {
	...
	private JTextField txtId,txtName,txtUnitprice, txtQuality, txtDE, txtCountry, txtTax, txtTrans;
	...
	public ExManagerUI(String title)
	{
		super(title);
...
// Tạo bảng và tiêu đề cho bảng
		...
		JLabel lblTitle=new JLabel("HOẠT ĐỘNG XUẤT KHẨU");
...
// Đặt tên và tạo bảng chứ thông tin chi tiết các loại hàng hóa
		...
		dtmProduct.addColumn("Ngày xuất ");
		dtmProduct.addColumn("Nơi xuất đi");
		dtmProduct.addColumn("Thuế xuất khẩu");
		dtmProduct.addColumn("Phương tiện");
...
// Tạo bảng cho người dùng nhập dữ liệu hàng hóa theo từng loại hàng
		...
		JLabel lblDE = new JLabel("Ngày xuất :");
		txtDE = new JTextField(20);
		pnDateImport.add(lblDE);
		pnDateImport.add(txtDE);
		pnProductDetail.add(pnDateImport);

		JPanel pnCountry = new JPanel();
		JLabel lblOG = new JLabel("Nơi xuất đi :");
		txtCountry = new JTextField(20);
		pnCountry.add(lblOG);
		pnCountry.add(txtCountry);
		pnProductDetail.add(pnCountry);
		...
		JLabel lblTrans = new JLabel("Phương tiện:");
		txtTrans = new JTextField(20);
		pnTrans.add(lblTrans);
		pnTrans.add(txtTrans);
		pnProductDetail.add(pnTrans);
...
// Set up các text feild thẳng hàng
		...
		lblTrans.setPreferredSize(lblProName.getPreferredSize());
	...
	private void showListProductIntoTable()
	{
		...
		for(Product p: selectedCate.getListPro())
		{
			...
			vec.add(p.getCountry());
			vec.add(p.CalTax()+"");
			vec.add(p.getTransportation());
	...
	private void doSaveProduct()
	{
		if(selectedCate != null)
		{
			...
			p.setDateExport(txtDE.getText());
			p.setTransportation((txtTrans.getText()));
